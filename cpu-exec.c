...
volatile bool tb_hashed = false;

static volatile TranslationBlock* theLastTb = NULL;
static volatile TranslationBlock* curTb = NULL;
static volatile target_ulong last_pc_record = 0;
...
static gboolean asid_pc_equal(gconstpointer v1, gconstpointer v2) {
	asid_pc asid_pc1 = *((asid_pc*) v1);
	asid_pc asid_pc2 = *((asid_pc*) v2);
	return asid_pc1.asid == asid_pc2.asid && asid_pc1.pc == asid_pc2.pc;
}

static guint asid_pc_hash(gconstpointer v) {
	asid_pc asid_pc1 = *((asid_pc*) v);
	return (guint) asid_pc1.pc * 33 + (guint) asid_pc1.asid;
}

static GHashTable* donePageHash = NULL;
#endif

#define LINE_SIZE 16
static void DumpHex(void* buf, size_t len, FILE* f) {
	char* p = buf;
	char line[LINE_SIZE];
	int k = 0;
	while (k < len) {
		line[k % LINE_SIZE] = p[k];
		if (k && ((k + 1) % LINE_SIZE == 0)) {
			fprintf(f, "%.2x |", p[k] & 0xff);
...
static int readMemToBuf(CPUState* cs, target_ulong addr, int len, uint8_t* buf) {
	hwaddr phys_addr;
	target_ulong page;
	int l;
	int ret = 0;
	while (len > 0) {
		...
		ret = cpu_physical_memory_rw_ex(phys_addr, buf, l, 0, true);
		if (ret < 0)
			break;
		len -= l;
		buf += l;
...
static void record_current_pc_page(CPUState* cs, target_ulong tbStart,
		target_ulong tbSize) {
	target_ulong asid = panda_current_asid(cs);
	char name_buf[1024];
	int ret;
	...
	if (rr_begin_code_record_names
			&& g_hash_table_size(rr_begin_code_record_names)) {
		if (!g_hash_table_contains(rr_begin_code_record_names, name_buf))
			return;
	}
...
#if 0
		if (donePageHash) {
			g_hash_table_destroy(donePageHash);
			donePageHash = g_hash_table_new_full(asid_pc_hash, asid_pc_equal,
					g_free, NULL);
		}
#endif
		NewTrace = 0;
	...
	struct stat st = { 0 };
	if (stat("./dyn_traces", &st) == -1) {
		mkdir("./dyn_traces", 0700);
	}
	snprintf(name_buf, sizeof(name_buf), "./dyn_traces/"TARGET_FMT_lx".log",
			asid);
	FILE* df = fopen(name_buf, "a+");
	if (!df)
		return;

	ret = readMemToBuf(cs, tbStart, tbSize, mem_buf);
	if (ret == -1) {
		fprintf(df, "\n"TARGET_FMT_lx":\n (None)", pc);
	} else {
		fprintf(df,
		...
		if (curTb) {
			...
			TranslationBlock* jlf = (TranslationBlock*) curTb->jmp_list_first;
			fprintf(df,
					"[curTb] orig_tb:"TARGET_FMT_lx" jmp_list_next[0]:"TARGET_FMT_lx" "
					"jmp_list_next[1]:"TARGET_FMT_lx" jmp_list_first:"TARGET_FMT_lx"\n",
					(target_ulong) (uintptr_t) curTb, (jln0 ? jln0->pc : 0),
		...
		if (theLastTb) {
			...
			TranslationBlock* jlf = (TranslationBlock*) curTb->jmp_list_first;
			fprintf(df,
					"[theLastTb] orig_tb:"TARGET_FMT_lx" jmp_list_next[0]:"TARGET_FMT_lx" "
					"jmp_list_next[1]:"TARGET_FMT_lx" jmp_list_first:"TARGET_FMT_lx"\n",
	...
	/* Create static traces */
#if 0
	target_ulong pc_page_start = pc & ~(TARGET_PAGE_SIZE - 1);
	asid_pc* ThePage = g_malloc0(sizeof(asid_pc));
	;
	ThePage->asid = asid;
	ThePage->pc = pc_page_start;
	if (!g_hash_table_insert(donePageHash, ThePage, NULL))
	return;
	memset(&st, 0, sizeof(st));
	if (stat("./stat_traces", &st) == -1) {
		mkdir("./stat_traces", 0700);
	}
	snprintf(name_buf, sizeof(name_buf), "./stat_traces/"TARGET_FMT_lx".log",
			asid);
	FILE* sf = fopen(name_buf, "a+");
	if (!sf)
	return;
	ret = readMemToBuf(cs, pc_page_start, TARGET_PAGE_SIZE, mem_buf);
	if (ret == -1) {
		fprintf(sf, "\n"TARGET_FMT_lx":\n (None)", pc);
	} else {
		fprintf(sf, "\n"TARGET_FMT_lx":\n", pc_page_start);
		DumpHex(mem_buf, TARGET_PAGE_SIZE, sf);
	}
	fclose(sf);
#endif
}
...
static tcg_target_ulong cpu_tb_exec(CPUState *cpu, TranslationBlock *itb) {
	...
	qemu_log_mask_and_addr(CPU_LOG_EXEC, itb->pc,
			...
			cpu->cpu_index, itb->pc, lookup_symbol(itb->pc));

#ifdef CONFIG_SOFTMMU
	{
		curTb = itb;
		if ((!tb_hashed) || TraceDetail) {
			// In LLVM mode we generate this more efficiently.
			if ((rr_mode == RR_CODE_RECORD) && !generate_llvm) {
				if (last_pc_record != itb->pc) {
					RR_DO_CODE_RECORD(
					/*action=*/
	...
	if (tb_exit > TB_EXIT_IDX1) {
		...
		qemu_log_mask_and_addr(CPU_LOG_EXEC, last_tb->pc,
				...
				last_tb->tc_ptr, last_tb->pc, lookup_symbol(last_tb->pc));
...
static inline TranslationBlock *tb_find(CPUState *cpu,
	...
	if (unlikely(
			!tb || tb->pc != pc || tb->cs_base != cs_base
					|| tb->flags != flags)) {
...
#ifdef CONFIG_SOFTMMU
	if (rr_mode != RR_REPLAY && panda_tb_chaining && 0) {
#endif
...
static inline void cpu_loop_exec_tb(CPUState *cpu, TranslationBlock *tb,
		TranslationBlock **last_tb, int *tb_exit, SyncClocks *sc) {
	uintptr_t ret;
...
int cpu_exec(CPUState *cpu) {
	...
	while (!cpu_handle_exception(cpu, &ret)) {
		...
		while (true) {
			...
			panda_bb_invalidate_done = panda_callbacks_after_find_fast(cpu, tb,
					panda_bb_invalidate_done, &panda_invalidate_tb);
			qemu_log_rr(tb->pc);
